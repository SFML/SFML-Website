...
<tr class="memdesc:"><td class="mdescLeft">&#160;</td><td class="mdescRight">Special type that can be passed to setParameter, and that represents the texture of the object being drawn.  <a href="structsf_1_1Shader_1_1CurrentTextureType.php#details">More...</a><br /></td></tr>
...
<p>There are two kinds of shaders: </p><ul>
...
<p>Like any C/C++ program, a shader has its own variables that you can set from your C++ application. <a class="el" href="classsf_1_1Shader.php" title="Shader class (vertex and fragment) ">sf::Shader</a> handles 5 different types of variables: </p><ul>
<li>floats </li>
...
<p>The value of the variables can be changed at any time with the various overloads of the setParameter function: </p><div class="fragment"><div class="line">shader.setParameter(<span class="stringliteral">&quot;offset&quot;</span>, 2.f);</div>
<div class="line">shader.setParameter(<span class="stringliteral">&quot;point&quot;</span>, 0.5f, 0.8f, 0.3f);</div>
<div class="line">shader.setParameter(<span class="stringliteral">&quot;color&quot;</span>, <a class="code" href="classsf_1_1Color.php">sf::Color</a>(128, 50, 255));</div>
...
<div class="line">shader.setParameter(<span class="stringliteral">&quot;texture&quot;</span>, <a class="code" href="classsf_1_1Shader.php#ac84c7953eec2e19358ea6e2cc5385b8d">sf::Shader::CurrentTexture</a>);</div>
</div><!-- fragment --><p>The special <a class="el" href="classsf_1_1Shader.php#ac84c7953eec2e19358ea6e2cc5385b8d" title="Represents the texture of the object being drawn. ">Shader::CurrentTexture</a> argument maps the given texture variable to the current texture of the object being drawn (which cannot be known in advance).</p>
<p>To apply a shader to a drawable, you must pass it as an additional parameter to the Draw function: </p><div class="fragment"><div class="line">window.draw(sprite, &amp;shader);</div>
...
<div class="line">states.<a class="code" href="classsf_1_1RenderStates.php#ad4f79ecdd0c60ed0d24fbe555b221bd8">shader</a> = &amp;shader;</div>
...
<p>Example: </p><div class="fragment"><div class="line">uniform <span class="keywordtype">float</span> myparam; <span class="comment">// this is the variable in the shader</span></div>
...
<p>Example: </p><div class="fragment"><div class="line">uniform vec2 myparam; <span class="comment">// this is the variable in the shader</span></div>
...
<p>Example: </p><div class="fragment"><div class="line">uniform vec3 myparam; <span class="comment">// this is the variable in the shader</span></div>
...
<p>Example: </p><div class="fragment"><div class="line">uniform vec4 myparam; <span class="comment">// this is the variable in the shader</span></div>
...
<p><em>name</em> is the name of the variable to change in the shader. The corresponding parameter in the shader must be a 3x1 vector (vec3 GLSL type).</p>
<p>Example: </p><div class="fragment"><div class="line">uniform vec3 myparam; <span class="comment">// this is the variable in the shader</span></div>
</div><!-- fragment --> <div class="fragment"><div class="line">shader.setParameter(<span class="stringliteral">&quot;myparam&quot;</span>, <a class="code" href="classsf_1_1Vector3.php">sf::Vector3f</a>(5.2f, 6.0f, -8.1f));</div>
</div><!-- fragment --><dl class="params"><dt>Parameters</dt><dd>
  <table class="params">
    <tr><td class="paramname">name</td><td>Name of the parameter in the shader </td></tr>
    <tr><td class="paramname">vector</td><td>Vector to assign </td></tr>
...
<p><em>name</em> is the name of the variable to change in the shader. The corresponding parameter in the shader must be a 4x1 vector (vec4 GLSL type).</p>
<p>It is important to note that the components of the color are normalized before being passed to the shader. Therefore, they are converted from range [0 .. 255] to range [0 .. 1]. For example, a <a class="el" href="classsf_1_1Color.php" title="Utility class for manipulating RGBA colors. ">sf::Color(255, 125, 0, 255)</a> will be transformed to a vec4(1.0, 0.5, 0.0, 1.0) in the shader.</p>
<p>Example: </p><div class="fragment"><div class="line">uniform vec4 color; <span class="comment">// this is the variable in the shader</span></div>
</div><!-- fragment --> <div class="fragment"><div class="line">shader.setParameter(<span class="stringliteral">&quot;color&quot;</span>, <a class="code" href="classsf_1_1Color.php">sf::Color</a>(255, 128, 0, 255));</div>
</div><!-- fragment --><dl class="params"><dt>Parameters</dt><dd>
  <table class="params">
    <tr><td class="paramname">name</td><td>Name of the parameter in the shader </td></tr>
    <tr><td class="paramname">color</td><td><a class="el" href="classsf_1_1Color.php" title="Utility class for manipulating RGBA colors. ">Color</a> to assign </td></tr>
  </table>
  </dd>
</dl>
...
<a class="anchor" id="a8599ee1348407025039b89ddf3f7cb62"></a>
<div class="memitem">
<div class="memproto">
      <table class="memname">
        <tr>
...
<p>Example: </p><div class="fragment"><div class="line">uniform mat4 matrix; <span class="comment">// this is the variable in the shader</span></div>
</div><!-- fragment --> <div class="fragment"><div class="line"><a class="code" href="classsf_1_1Transform.php">sf::Transform</a> transform;</div>
<div class="line">transform.<a class="code" href="classsf_1_1Transform.php#ab54f6c8070cc05e2afcb3145fbf4395a">translate</a>(5, 10);</div>
...
<p>To use the texture of the object being draw, which cannot be known in advance, you can pass the special value <a class="el" href="classsf_1_1Shader.php#ac84c7953eec2e19358ea6e2cc5385b8d" title="Represents the texture of the object being drawn. ">sf::Shader::CurrentTexture</a>: </p><div class="fragment"><div class="line">shader.setParameter(<span class="stringliteral">&quot;the_texture&quot;</span>, <a class="code" href="classsf_1_1Shader.php#ac84c7953eec2e19358ea6e2cc5385b8d">sf::Shader::CurrentTexture</a>).</div>
</div><!-- fragment --><dl class="params"><dt>Parameters</dt><dd>
  <table class="params">
...
</div><!-- fragment --> <div class="fragment"><div class="line">shader.setParameter(<span class="stringliteral">&quot;current&quot;</span>, <a class="code" href="classsf_1_1Shader.php#ac84c7953eec2e19358ea6e2cc5385b8d">sf::Shader::CurrentTexture</a>);</div>
</div><!-- fragment --><dl class="params"><dt>Parameters</dt><dd>
  <table class="params">
    <tr><td class="paramname">name</td><td>Name of the texture in the shader </td></tr>
  </table>
...
<p>Definition at line <a class="el" href="Shader_8hpp_source.php#l00080">80</a> of file <a class="el" href="Shader_8hpp_source.php">Shader.hpp</a>.</p>
