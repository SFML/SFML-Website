...
<p>
    ...
    will be created. You can generate them directly in the source tree (ie. the SFML root directory), but it will then be polluted with a lot
...
<p>
    To compile for Android, you need to add an environment variable pointing on the location of your NDK directory. This folder contains all toolchains
    needed to build for each supported architecture. You'll also need to specify our custom CMake toolchain located in the source directory, named
    <em>android.toolchain.cmake</em> and located under cmake/toolchains directory). To do that, define CMAKE_TOOLCHAIN_FILE in the command line.

</p>
<pre><code class="no-highlight">&gt; SET ANDROID_NDK=path\to\ndk\directory
&gt; cmake -DCMAKE_TOOLCHAIN_FILE=sfml-src\cmake\toolchains\android.toolchain.cmake
</code></pre>
<pre><code class="no-highlight">&gt; export ANDROID_NDK=/path/to/ndk/directory
&gt; cmake -DCMAKE_TOOLCHAIN_FILE=sfml-src/cmake/toolchains/android.toolchain.cmake
</code></pre>
<p class="important">
    On Windows, don't use Visual Studio compilers if you need to compile for Android. Instead, use MinGW.
    Also, you don't want a static build, usage of shared libraries is mandatory for Android.
</p>
<p>
...
<p class="important">
    For Android, do not select any generator. Instead, use our CMake custom toolchain named <em>android.toolchain.cmake</em> located in the source
    directory under <em>cmake/toolchains</em> directory. To do that, select "Specify toolchain file for cross-compiling".
</p>
<p>
    ...
    <tbody>
        ...
        <tr class="two">
            ...
            <td>
                ...
                Please make sure that /Library/Developer/Xcode/Templates/SFML exists and is writeable.
        ...
        <tr class="one">
            <td><code>ANDROID_ABI<br/>(Android only)</code></td>
            <td>
                This variable specifies for which architectures SFML should be compiled. The default value is
                "armeabi" but the following choices are available: "armeabi", "armeabi-v7a", "x86", "mips".
            </td>
        </tr>
        <tr class="one">
            <td><code>ANDROID_NATIVE_API_LEVEL<br/>(Android only)</code></td>
            <td>
                This value corresponds to the Android OS version you want to target. Note that selecting version 9
                will generate binaries compatible for all device SFML supports.
            </td>
        </tr>
    </tbody>
</table>
