...
<p>
    All SFML classes (sprites, text, shapes) use the same interface for transformations: <?php class_link('Transformable') ?>. This base class defines a simple API to move,
    rotate and scale your entities. It doesn't provide maximum flexibility, but it rather defines an interface which is easy to understand and to use, and which covers 99%
    of use cases -- for the other 1%, see the last chapters.
</p>
<p>
    ...
    and <strong>origin</strong>; they all have their getter and setter. These transformation components are all independant from each other: if you want to change the
...
<h3>Scale</h3>
<p>
    The scale factor allows to resize the entity. The default scale is 1, less than 1 makes the entity smaller, more than 1 makes it bigger. Negative scales are also allowed,
...
<h3>Origin</h3>
<p>
    The origin is the center point of the three other transformations. The position is the position of the origin, the rotation is made around the origin, and the scale is
    applied around the origin too. By default it is the top-left corner of the entity (point (0, 0)), but you can change it so that it is its center, or another corner
    for example.
</p>
<p>
    To keep things simple, there's only one origin for the three transformation components. Which means that you can't, for example, position an entity relatively to its
    top-left corner while rotating it around its center. If you need to do such things, have a look at the next chapters.
</p>
<pre><code class="cpp">// 'entity' can be a sf::Sprite, a sf::Text, a sf::Shape or any other transformable class
...
<p>
    Note that changing the origin also changes the visual position of the entity, although its position property is still the same. If you don't understand why,
    read this tutorial one more time!
</p>

<?php h2('Transforming your own classes') ?>
<p>
    <?php class_link('Transformable') ?> is not only made for SFML classes, it can also be a base (or member) of you own classes.
</p>
...
<p>
    To make use of the final transform of the entity (most likely to draw it), call the <code>getTransform</code> function. This function returns a
    <?php class_link('Transform') ?>; see below for more explanations about it, and how to use it to transform an SFML entity.
</p>
<p>
    If you don't need/want the complete set of functions provided by the <?php class_link('Transformable') ?> interface, don't hesitate to use it as a member instead,
    and to provide your own functions on top of it. It is not abstract so there's no problem to instanciate it instead of using it as a base class.
</p>

<?php h2('Custom transforms') ?>
<p>
    The <?php class_link('Transformable') ?> class is easy to use, but it is also limited. Some users need more power, they need to specify a final transformation as
    a custom combination of individual transformations. For this kind of users, a lower-level class is available: <?php class_link('Transform') ?>. It is nothing more than
    a 3x3 matrix, so it can represent any transformation in the 2D space.
</p>
<p>
...
<p>
    If your entity is a <?php class_link('Transformable') ?> (sprite, text, shape), with its own internal transform, then both are combined to produce the final transform.
</p>
...
<p>
    The bounding box is very useful to implement collision detection: it is very fast to check against a point or another axis-aligned rectangle, and it is close enough
    to the real entity to provide a good approximation.
</p>
...
<p>
    The function is named <code>getGlobalBounds</code> because it returns the bounding box of the entity in the global coordinates system, ie. with all its
    transformations (position, rotation, scale) applied.
</p>
<p>
    There's another function that returns the bounding box of the entity in its <em>local</em> coordinates system (without transformations applied):
...
<p>
    With the custom transforms seen previously, it's now easy to implement a hierarchy of objects, where children are transformed relatively to their parent.
    All you have to do is to pass the combined transform from parent to children when you draw them, until you reach the final drawable entities (sprites, texts, shapes,
