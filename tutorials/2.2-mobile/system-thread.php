...
<p>
    A thread is basically a sequence of instructions that run in parallel of other threads. Every program is made of at least one thread: the main one, which
    runs your <code>main()</code> function. Programs that only use the main thread are <em>monothreaded</em>, if you add one or more threads they become
    <em>multithreaded</em>.
</p>
<p>
...
<p>
    If you work with compilers that support the new standard and its <code>&lt;thread&gt;</code> header, forget about SFML classes and use it -- it will
    be much better. But if you work with a pre-2011 compiler, or plan to distribute your code and want it to be fully portable, then SFML threading
...
<p>
    The entry point of the thread, ie. the function that will be run in the thread, must be passed to the constructor of <?php class_link("Thread") ?>.
...
<p>
    ...
    delay its construction by allocating it on the heap with <code>new</code>.
</p>
...
<p>
    A thread automatically stops when its function returns. If you want to wait for a thread to finish from another thread, you can call its
...
<p>
    The <code>wait</code> function is also implicitely called by the destructor of <?php class_link("Thread") ?>, so that a thread cannot remain alive
...
<p>
    All the threads of a program share the same memory, they can access all the variables of the program. It is very convenient but also dangerous:
    ...
    And if the operation is not <em>thread-safe</em>, the result is undefined (ie. it might crash or corrupt data).
</p>
<p>
    ...
    are mutexes, semaphores, wait conditions and spin locks. They are all variants of the same concept: they protect a piece of code by allowing only
    certain threads to access it while blocking the others.
<p>
    The most basic (and used) primitive is the mutex. Mutex stands for "MUTual EXclusion": it allows only one thread at a time to access the pieces of
    code that it surrounds. Let's see how they can bring some order to the example above:
</p>
<pre><code class="cpp">#include &lt;SFML/System.hpp&gt;
#include &lt;iostream&gt;
...
<p>
    ...
    (similarly to <code>sf::sleep</code>, no CPU is consumed by the sleeping thread). When the first thread finally unlocks the mutex, the second thread
    is awoken and is allowed to lock the mutex and print its text bloc in turn. Thus lines of text appear sequentially in the console instead of being
    mixed.
</p>
...
<p>
    ...
    thrown while a mutex is locked? It never gets a chance to be unlocked, and remains locked forever. And all threads that will try to lock it will
    block forever; your whole application could freeze. Pretty bad result.
</p>
<p>
    To make sure that mutexes are always unlocked in an environment where exceptions can be thrown, SFML provides a RAII class to wrap
    them: <?php class_link("Lock") ?>. It locks the mutex in its constructor, and unlocks it in its destructor.
...
<p>
    ...
    when the threaded function ends. But it is not what happens: the threaded function appears to block the main thread, as if the thread wasn't working.
</p>
<p>
    The reason? The <?php class_link("Thread") ?> instance is local to the <code>startThread()</code> function and is therefore immediately destroyed,
...
<p>
    So, don't forget this: you must manage your <?php class_link("Thread") ?> instance so that it lives as long as the threaded function runs.
</p>
