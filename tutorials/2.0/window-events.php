...
<p class="important">
    Sometimes, people try to use the <code>KeyPressed</code> event to implement smooth movement. Doing so will <em>not</em> produce smooth movements,
    because when you hold a key you only get a few events (remember, the repeat delay). Smooth movements can only be achieved by using real-time
    keyboard input with <?php class_link("Keyboard") ?> (see the
