...
<?php h2('Introduction') ?>
<p>
    As you learnt in the previous tutorials, the window module of SFML provides an easy way to open an OpenGL window and handle its events but it
    doesn't help when it comes to draw something. The only option which is left to you is to use the powerful-yet-complex-and-low-level OpenGL API.
</p>
<p>
...
<p>
    To draw the entities provided by the graphics module, you must use a specialized window class: <?php class_link("RenderWindow") ?>. This class derives
    ...
    controlling the framerate, mixing with OpenGL, etc.) is applicable to <?php class_link("RenderWindow") ?>.
</p>
<p>
    ...
    of these functions: <code>clear</code> and <code>draw</code>. They are as simple as their name: <code>clear</code> clears the whole window with
    the chosen color, and <code>draw</code> draws whatever object you give to it.
</p>
<p>
...
<p>
    Calling <code>clear</code> before drawing anything is mandatory, otherwise you may keep undefined pixels from previous frames. The only exception is
    when you cover the entire window with what you draw, so that no pixel is left undrawn; in this case you can avoid calling <code>clear</code> (although
    it won't make a big difference on performances).
</p>
<p>
...
<p class="important">
    ...
    Modern graphics chipsets and APIs are <em>really</em> made for repeated clear/draw/display cycles where everything is completely refreshed at each iteration of
...
<p>
    SFML provides four kinds of drawable entities: three of them are ready to be used (<em>sprites</em>, <em>texts</em> and <em>shapes</em>), the last
...
<p>
    ...
    you can copy it to your own <?php class_link("Texture") ?> instance.
</p>
<p>
    ...
    for more details). If you use OpenGL to draw to the render-texture, you can have a depth buffer by using the third optional argument of the <code>create</code>
    function.
</p>
...
<p>
    ...
    another thread; that's because a window (more precisely its OpenGL context) cannot be active in multiple threads at the same time.
</p>
...
<p>
    Remember to always create the window and handle its events in the main thread, for maximum portability, as explained in the
