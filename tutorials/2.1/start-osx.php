...
<ul>
    <li>an Intel Mac with Leopard or later</li>
    <li>and <a href="http://developer.apple.com/xcode/" title="Download Xcode">Xcode</a> (preferably the fourth version of the IDE which is available on the <em>App Store</em>)</li>
</ul>
<p class="important">
...
<p class="important">
    If you have Xcode 5, you must use <a href="../../download.php" title="Go to the download page">the new version of the templates</a>.
</p>

<h3>Binaries: dylib vs framework</h3>
<p>
    SFML is available in two formats on Mac OS X. You have the <em>dylib</em> libraries on one hand and the <em>framework</em> bundles on the other hand.
...
<p>
    SFML is provided with two templates for Xcode 4 which allow you to create very quickly and easily new application projects. These templates can create custom projects:
    you can select which modules your application requires, if you want to use SFML as dylib or as frameworks, which compiler you want to use and you can choose between
...
<h3>C++ 11</h3>
<p>
    Apple ships a custom version of <tt>clang</tt> and <tt>libc++</tt> with Xcode 4 that support part of the C++11 standard (i.e. new features are not yet all implemented).
    If you plan to use C++11's new functionalities, you need to configure your project to use <tt>clang</tt> and <tt>libc++</tt>. Also, make sure to download the right
    version (more details follow).

<?php h2('Installing SFML') ?>
<p>
    First of all you will need to download the SFML SDK which is available on the <a href="../../download.php" title="Go to the download page">download page</a>.
</p>
<p class="important">
    If you want to target Mac OS X Leopard (10.5) you have to download the "GCC" version. Keep in mind that this version is not compatible with C++11 features.
    If you want to use those features you need the "Clang" version, which is compatible with Mountain Lion (10.8) and later.
</p>
<p>
...
<ul>
    <li>
        ...
        As stated at the beginning of this document, SFML is available either as dylibs or as frameworks. We recommend using the frameworks but both can be installed on
        one system.
        <ul>
            <li>dylib<br />
...
<p>
    SFML is shipped with an <tt>install.sh</tt> script that will install everything for you.
    It will also make sure that previous versions of SFML, especially frameworks, are not removed from your system so that your applications can still be executed without modifications.
    You just need to open a terminal in the directory of the downloaded SDK and run <code>./install.sh</code>.
    The script will then prompt for your password and install everything in the right place.
</p>

<?php h2('Create your first SFML program') ?>
<p>
    We provide two templates for Xcode. <tt>SFML CLT</tt> generates a project for a classic terminal app whereas <tt>SFML App</tt> creates a project for an application
...
<p class="important">
    A few words about the templates settings. If you choose an incompatible option for <tt>C++ Compiler and Standard Library</tt> you will end up with linker errors. Make sure you follow this guideline:
</p>
<ul>
    <li>If you downloaded the "GCC" version from the download page, you should select <tt>C++98 with GCC and libstdc++ and target 10.5</tt>.</li>
...
<p>
    The other files of the project are not very relevant for us here. However, please note that the SFML dependencies of your project are added to your application bundle,
