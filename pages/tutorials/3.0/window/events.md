...
SFML uses `std::variant` under the hood to implement a type-safe API for events.
There are two main ways to use this API.
Check out the EventHandling example program to see these methods in practice.
...
This event only represents a close request.
The window is not yet closed when the event is received.
...
There's no data associated with this event.
...
If a key is held, multiple `KeyPressed` events will be generated, at the default operating system delay (ie. the same delay that applies when you hold a letter in a text editor).
...
This event is triggered even if the window isn't focused.
However, it is triggered only when the mouse moves within the inner area of the window, not when it moves over the title bar or borders.
...
There's no data associated with these events.
